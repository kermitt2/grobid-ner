...
public class NERTrainerRunner {
    ...
    public static void main(String[] args) {
        final String helpUsage = "Usage: {0 - train, 1 - evaluate, 2 - split, train and evaluate} {ner,nerfr,nersense} -gH /path/to/Grobid/home -s { [0.0 - 1.0] - split ratio, optional}";
        if (args.length < 4) {
        ...
        if ((mode == RunType.SPLIT) && (args.length < 6)) {
            throw new IllegalStateException("Missing arguments for " + RunType.SPLIT + ".\n" + helpUsage);
        }
        ...
        for (int i = 0; i < args.length; i++) {
            if (args[i].equals("-gH")) {
                ...
                try {
                    split = Double.parseDouble(splitRatio);
                } catch (Exception e) {
                    throw new IllegalStateException("Invalid split value: " + splitRatio);
                }
