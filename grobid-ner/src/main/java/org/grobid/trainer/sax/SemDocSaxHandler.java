...
/**
 * SAX parser for SemDoc XML format (can be used for the idilia and reuters semdoc corpora)
 *
...
public class SemDocSaxHandler extends DefaultHandler {
    ...
    private boolean inSenseInformation = false;
    ...
    private boolean toRealign = true;
    ...
    public SemDocSaxHandler(List<List<String>> text) {
        ...
        textVector = text;
        annotatedTextVector = new ArrayList<>();
    }
    ...
    public void setToRealign(boolean toR) {
        toRealign = toR;
    }
    ...
    public void endElement(String uri, String localName, String qName) throws SAXException {
        ...
        if (qName.equals("sensesInfo")) {
            inSenseInformation = false;
        }

        if (qName.equals("frag")) {
            while (currentTokenLength > 0) {
                if (currentSenseKey != null) {
                    accumulatedSenseKeys.add(new Pair<>(currentSenseKey, srcString));
                } else {
                    accumulatedSenseKeys.add(new Pair<>(null, srcString));
                }
                currentTokenLength--;
            }
            ...
            // we can use the identified dependency tokens to try to re-segment the local input text
            ...
            if (currentVectorPos < textVector.size()) {
                if (toRealign) {
                    ...
                    textVector.set(currentVectorPos, newSent);
                }
    ...
    public void startElement(String namespaceURI, String localName, String qName, Attributes atts)
        ...
        if (qName.equals("sensesInfo")) {
            inSenseInformation = true;
        } else if (inSenseInformation && qName.equals("sense")) {
            isNE = atts.getValue("isne") != null;
            currentSenseKey = atts.getValue("fsk");
            currentCoarseSenseKey = atts.getValue("csk");
        ...
        if (inSenseInformation == false) {
            if (qName.equals("fs")) {
                currentSenseKey = atts.getValue("sk");
                currentFineConfidence = atts.getValue("pc");
            } else if (qName.equals("cs")) {
                currentCoarseSenseKey = atts.getValue("sk");
                currentCoarseConfidence = atts.getValue("pc");
            } else if (qName.equals("frag")) {
                covered = false;
                currentTokenLength = Integer.parseInt(atts.getValue("len"));
            } else if (qName.equals("sent")) {
                currentTokenPos = 0;
                currentSentenceLength = Integer.parseInt(atts.getValue("len"));

                if (currentSentenceLength != 0) {
                    if (tokensFrag == null && toRealign)
                        tokensFrag = tokensF();
                    accumulatedSenseKeys = new ArrayList<>();
                }

            } else if (qName.equals("dep") && toRealign) {
                String dest = atts.getValue("dest");
                if (!tokensFrag.contains(dest)) {
                    tokensFrag.add(dest);
                }

                String src = atts.getValue("src");
                if (!tokensFrag.contains(src)) {
                    tokensFrag.add(src);
                }
                srcString = src;
            } else if(qName.equals("txt")) {
